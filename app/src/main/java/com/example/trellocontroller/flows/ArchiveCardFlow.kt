...
class ArchiveCardFlow(private val dependencies: ArchiveCardFlowDependencies) {
    ...
    fun start(initialDataFromAI: JSONObject?) {
        ...
        if (pendingBoardNameFromAI != null) {
            ...
            dependencies.speakWithCallback("In welchem Board befindet sich die Karte, die du archivieren möchtest?") {
    ...
    fun handleSpokenInput(input: String) {
        ...
        when (currentState) {
            ...
            State.CONFIRM_ARCHIVAL -> handleConfirmArchival(input)
    ...
    private fun proceedToAskCardName() {
        ...
        if (pendingCardNameFromAI != null) {
            ...
            dependencies.speakWithCallback("Wie heißt die Karte, die du archivieren möchtest?") {
            ...
            onResult = { cards ->
                ...
                when (matchingCards.size) {
                    1 -> {
                        ...
                        dependencies.updateUiContext(buildContextText())
                        proceedToConfirmArchival()
                    }
                    0 -> {
                        dependencies.speakWithCallback("Ich konnte keine Karte mit dem Namen '$cardNameInput' in der Liste '$currentListName' finden. Möchtest du es erneut versuchen oder eine andere Karte suchen?") {
                            dependencies.startSpeechInput("Erneut versuchen oder andere Karte?")
                        }
                        currentState = State.CONFIRM_CARD_NOT_FOUND
                    }
                    else -> {
                        // Mehrere Karten gefunden, nimm die erste (oder erweitern für Auswahl)
                        currentCardId = matchingCards.first().second
                        currentCardName = matchingCards.first().first
                        dependencies.speakWithCallback("Ich habe mehrere Karten mit dem Namen '$cardNameInput' gefunden. Ich nehme die erste. Möchtest du fortfahren?") {
                            proceedToConfirmArchival()
                        }
                    }
                }
    ...
    private fun handleCardNotFoundConfirmation(input: String) {
        if (input.lowercase().contains("erneut") || input.lowercase().contains("ja")) {
            ...
            dependencies.speakWithCallback("Okay, Vorgang abgebrochen.") {
                dependencies.onFlowComplete("Archivierung abgebrochen.", false)
                resetState()
            }
        }
    }

    private fun proceedToConfirmArchival() {
        currentState = State.CONFIRM_ARCHIVAL
        dependencies.updateUiContext(buildContextText())
        val confirmMsg = "Soll die Karte '$currentCardName' archiviert werden (Liste: '$currentListName', Board: '$currentBoardName')?"
        dependencies.speakWithCallback(confirmMsg) {
            dependencies.startSpeechInput("Ja oder Nein?")
        }
    }

    private fun handleConfirmArchival(input: String) {
        if (input.lowercase().contains("ja")) {
            val cardId = currentCardId ?: return flowError("Karten ID fehlt.")
            dependencies.archiveCardOnTrello(cardId,
                onSuccess = {
                    val successMsg = "Karte '$currentCardName' wurde erfolgreich archiviert."
                    dependencies.onFlowComplete(successMsg, true)
                    resetState()
                },
                onError = { errMsg ->
                    dependencies.onFlowComplete("Fehler beim Archivieren der Karte: $errMsg", false)
                    resetState()
                }
            )
        } else {
            dependencies.speakWithCallback("Okay, Karte nicht archiviert.") {
                dependencies.onFlowComplete("Archivierung abgebrochen.", false)
                resetState()
            }
        }
    ...
    private fun flowError(message: String) {
        dependencies.onFlowComplete("Interner Fehler im Archivierungsflow: $message", false)
        resetState()
    }
    ...
    private fun buildContextText(): String {
        val sb = StringBuilder("Aktion: Karte archivieren")
        ...
        val nextStep = when (currentState) {
            ...
            State.CONFIRM_ARCHIVAL -> "Archivierung bestätigen? (Ja/Nein)"
            State.IDLE -> ""
        }
